...
function ProcessScript()
   ...
   local file = io.prepareMeshFile('mesh', '$SIMULATIONDIR/out/$SIMULATIONNAME_' .. userParams.n, 'nf', {'u', 'Rf'}, {'S', 'E'}, {split = true, saveDisplacements = true})
   local nsteps = solverOptions.stepsMax
   local loadFile = io.open(translatePath('$SIMULATIONDIR') .. '/out/output_' .. userParams.n .. '.txt', "w+")
   assert(loadFile)
   loadFile:write("GeMA - Elastico - E = " .. userParams.E/1000 .. " Gpa\n")
